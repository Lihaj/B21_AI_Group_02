...
Feature: Admin - Plants API
  ...
  Scenario: Update plant with valid data as Admin
    When I send a PUT request to update plant with id "1" with plant data
      | name        | price | quantity |
      | Snake Plant | 30    | 70       |
  ...
  Scenario: Delete existing plant as Admin
    ...
    When I send a DELETE request to delete plant with id "1"
    Then the response status should be 204
    And the plant with id "1" should not exist
